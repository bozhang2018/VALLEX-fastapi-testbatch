...
class VALLE(VALLF):
    ...
    def inference(
        ...
        text_len = x_lens.max()
        ######################
        # test: duplicate x, make it with dimension (2, T, 8), where 2 is batch_size
        y = y.repeat(2, 1, 1)
        #######################
        prompts = y
        ...
        while True:
            ...
            if (
                ...
                print(f"Model is running ar decoder.")
        ...
        if self.prefix_mode in [2, 4]:  # Exclude enrolled_phonemes
            ...
            #####################
            # remove the assert to allow batch_size > 1
            # assert text.shape[0] == 1
            ########################

        x = self.nar_text_embedding(text)
        print(f"Model is running nar decoder.")
        # Add language embedding
        prompt_language_id = torch.LongTensor(np.array([self.language_ID[prompt_language]])).to(x.device)
        if isinstance(text_language, str):
            text_language_id = torch.LongTensor(np.array([self.language_ID[text_language]])).to(x.device)
